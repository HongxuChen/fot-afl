...
static void edit_params(int argc, char** argv) {
  ...
  /* On MacOS X, the Xcode cctool 'as' driver is a bit stale and does not work
     ...
     To work around this, when using clang and running without FOT_AS
...
int main(int argc, char** argv) {
  ...
  u32 rand_seed;
  int status;
  u8* inst_ratio_str = getenv("FOT_INST_RATIO");
  ...
  if (argc < 2) {
    ...
    SAYF("\n"
         ...
         "set FOT_INST_RATIO to a value less than 100 in order to reduce the odds of\n"
  ...
  if (inst_ratio_str) {
    ...
    if (sscanf(inst_ratio_str, "%u", &inst_ratio) != 1 || inst_ratio > 100) 
      FATAL("Bad value of FOT_INST_RATIO (must be between 0 and 100)");
  ...
  if (getenv("FOT_USE_ASAN") || getenv("FOT_USE_MSAN")) {
    sanitizer = 1;
    inst_ratio /= 3;
  }
  ...
  if (!getenv("FOT_KEEP_ASSEMBLY")) unlink(modified_file);
